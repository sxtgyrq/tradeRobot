...
{
    public class DALHourRecord
    {
        public static bool Insert(dataItem data)
        {
            bool success;
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var nextItem = new dataItem()
                                {
                                    dateTime = data.dateTime.AddHours(1)
                                };
                                var previous = new dataItem()
                                {
                                    dateTime = data.dateTime.AddHours(-1)
                                };
        ...
        private static int InsertItem(dataItem data, MySqlTransaction tran, MySqlConnection con)
        {
            int rowNum;
            string sQL = @"INSERT INTO hourrecord(recordDateTime,
openValue,
highValue,
lowValue,
closeValue,
...
@volumeValue);";
        ...
        private static int updateItem(dataItem data, MySqlTransaction tran, MySqlConnection con)
        {
            int rowNum;
            string sQL = @"UPDATE hourrecord SET openValue=@openValue,highValue=@highValue,lowValue=@lowValue,closeValue=@closeValue,volumeValue=@volumeValue WHERE recordDateTime=@recordDateTime";
            //                // long moneycount;
            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
            {
                command.Parameters.AddWithValue("@openValue", data.openValue);
        ...
        private static bool exit(dataItem data, MySqlTransaction tran, MySqlConnection con)
        {
            var sQL = $"SELECT recordDateTime FROM hourrecord WHERE recordDateTime='{data.dateTime.ToString("yyyy-MM-dd HH:mm:ss")}'";
            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
            {
        ...
        public static bool Update(dataItem data)
        {
            bool success;
            using (MySqlConnection con = new MySqlConnection(Connection.ConnectionStr))
            {
            ...
            using (MySqlConnection con = new MySqlConnection(Connection.ConnectionStr))
            {
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            var sQL = $"SELECT * FROM hourrecord WHERE recordDateTime='{data.dateTime.ToString("yyyy-MM-dd HH:mm:ss")}'";
                            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
                            {
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            var sQL = $"SELECT * FROM hourrecord WHERE recordDateTime>='{dataParameter.ToString("yyyy-MM-dd HH:mm:ss")}' and recordDateTime<'{endData.ToString("yyyy-MM-dd HH:mm:ss")}' order by recordDateTime";
                            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
                            {
        ...
        public static List<dataItem> GetAll(MySqlConnection con, MySqlTransaction tran)
        {
            List<dataItem> datas = new List<dataItem>();
            {
                var sQL = $"SELECT * FROM hourrecord order by recordDateTime";
                using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
                {
        ...
        public static List<dataItem> GetAfter(dataItem conditionItem, int nextListCount, MySqlTransaction tran, MySqlConnection con)
        {
            ...
            var sQL = $"SELECT * FROM hourrecord WHERE recordDateTime>'{timeLimit.ToString("yyyy-MM-dd HH:mm:ss")}' order by recordDateTime ASC  LIMIT 0,{nextListCount};";
            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
            {
        ...
        public static List<dataItem> GetPrevious(DateTime baseHourRecord, int previousListCount, MySqlTransaction tran, MySqlConnection con)
        {
            var timeLimit = baseHourRecord.AddMinutes(-1);
            List<dataItem> datas = new List<dataItem>();
            //SELECT * FROM hourrecord WHERE recordDateTime>'{timeLimit.ToString("yyyy-MM-dd HH:mm:ss")}' order by recordDateTime ASC  LIMIT 0,48;
            var sQL = $"SELECT * FROM hourrecord WHERE recordDateTime<'{timeLimit.ToString("yyyy-MM-dd HH:mm:ss")}' order by recordDateTime DESC LIMIT 0,{previousListCount};";
            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
            {
                using (var reader = command.ExecuteReader())
                {
